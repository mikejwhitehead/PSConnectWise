...
Describe -Tag 'ReqPriorityPermission' 'CWServicePriority' {
	...
	Context 'Get-CWServicePriority' {
		...
		It 'gets priority and checks for the id field' {
			...
			$priority = Get-CWServicePriority -PriorityID $priorityID -BaseApiUrl $pstrSvrUrl -CompanyName $pstrSvrCompany -PublicKey $pstrSvrPublic -PrivateKey $pstrSvrPrivate;
		...
		It 'gets priorities by passing array of priority ids to the -PriorityID param' {
			...
			$priorities = Get-CWServicePriority -PriorityID $priorityIDs -BaseApiUrl $pstrSvrUrl -CompanyName $pstrSvrCompany -PublicKey $pstrSvrPublic -PrivateKey $pstrSvrPrivate;
		...
		It 'gets list of priorities that were piped to the cmdlet' {
			$priorityIDs = $pstrPriorityIDs;
			$priorities = $priorityIDs | Get-CWServicePriority -BaseApiUrl $pstrSvrUrl -CompanyName $pstrSvrCompany -PublicKey $pstrSvrPublic -PrivateKey $pstrSvrPrivate;
			$priorities | Measure-Object | Select -ExpandProperty Count | Should Be $priorityIDs.Count;		
		}
		
		It 'gets priority based on the -Filter param' {
			$filter = "id = $pstrPriorityID";
			$priority = Get-CWServicePriority -Filter $filter -BaseApiUrl $pstrSvrUrl -CompanyName $pstrSvrCompany -PublicKey $pstrSvrPublic -PrivateKey $pstrSvrPrivate;
		...
		It 'gets priority based on the -Filter param and uses the SizeLimit param' {
			...
			$sizeLimit =  2;
			$priorities = Get-CWServicePriority -Filter $filter -SizeLimit $sizeLimit -BaseApiUrl $pstrSvrUrl -CompanyName $pstrSvrCompany -PublicKey $pstrSvrPublic -PrivateKey $pstrSvrPrivate;
		...
		It 'gets priorities and sorts priority id by descending piping cmdlet through Sort-Object cmdlet' {
			$priorityIDs = $pstrPriorityIDs;
			$priorities = Get-CWServicePriority -PriorityID $priorityIDs -BaseApiUrl $pstrSvrUrl -CompanyName $pstrSvrCompany -PublicKey $pstrSvrPublic -PrivateKey $pstrSvrPrivate | Sort -Descending id;
