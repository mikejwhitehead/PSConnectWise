...
Describe 'CWServiceTicket' {
	...
	Context 'Get-CWServiceTicket' {
		...
		It 'wildcard search using Summary parameter with SizeLimit parameter' {
			$sizeLimit = 5;
			# CW using semi-uses regex for searching so replaced the control squard brakets with stars
			$ticketSummary = ($pstrSharedValues['ticket'].summary).Replace("[","*").Replace("]","*");
	...
	Context "New-CWServiceTicket"  {
		...
		It "create a new service ticket and check for the ticket number" {
			...
			$pstrSharedValues.Add("newTicket", $ticket);
	...
	Context "Update-CWServiceTicket"  {
		...
		$pstrTicket    = $pstrSharedValues["newTicket"];

		It "change the subject of a ticket" {
			$ticketID = $pstrTicketID;
			$statusID = $pstrStatusID;
			$newSubject = "Changed " + $pstrTicket.summary
			$ticket = Update-CWServiceTicket -ID $ticketID -Subject $newSubject;
			$ticket.summary -eq $newSubject | Should Be $true; 
		}
